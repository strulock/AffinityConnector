...
export function registerListTools(server: McpServer, api: ListsApi): void {
  ...
  server.tool(
    ...
    {
      list_id: z.coerce.number().int().min(1).describe('Affinity list ID (use get_lists to find IDs)'),
      limit: z.coerce.number().int().min(1).max(100).default(25).describe('Max entries to return'),
  ...
  server.tool(
    ...
    {
      list_entry_id: z.coerce.number().int().min(1).describe('List entry ID (from get_list_entries results)'),
    },
    async ({ list_entry_id }) => {
      try {
  ...
  server.tool(
    ...
    {
      field_value_id: z.coerce.number().int().min(1).describe('Field value ID to delete (from get_field_values results)'),
    },
  ...
  server.tool(
    ...
    {
      list_id: z.coerce.number().int().min(1).describe('List ID to add to (from get_lists)'),
      entity_id: z.coerce.number().int().min(1).describe('ID of the person, org, or opportunity to add'),
      entity_type: z.coerce.number().int().describe('Entity type: 0 = person, 1 = organization, 8 = opportunity'),
    },
  ...
  server.tool(
    ...
    {
      list_id: z.coerce.number().int().min(1).describe('List ID containing the entry (from get_lists)'),
      list_entry_id: z.coerce.number().int().min(1).describe('List entry ID to remove (from get_list_entries)'),
    },
  ...
  server.tool(
    ...
    {
      list_id: z.coerce.number().int().min(1).describe('List ID containing the entry (from get_lists)'),
      list_entry_id: z.coerce.number().int().min(1).describe('List entry ID to update (from get_list_entries)'),
      fields: z
        .array(z.object({
          field_id: z.coerce.number().int().min(1).describe('Field ID (from get_field_definitions)'),
          value: z.union([z.string(), z.coerce.number(), z.boolean(), z.null()]).describe('New field value'),
        }))
        .min(1)
        .max(100)
  ...
  server.tool(
    ...
    {
      list_id: z.coerce.number().int().min(1).describe('List ID (from get_lists)'),
      view_id: z.coerce.number().int().min(1).describe('Saved view ID (from get_saved_views)'),
      limit: z.coerce.number().int().min(1).max(100).default(25).describe('Max entries to return'),
      page_token: z.string().optional().describe('Pagination token from a previous call'),
    },
